...
def model_init_fn(inputs, is_training):
    ...
    avg_pool = tf.layers.average_pooling2d(relu3, [5,5], [1,1])
    avg_pool = tf.layers.flatten(avg_pool)
    
    fc_1 = tf.layers.dense(avg_pool, 50)
    fc_2 = tf.layers.dense(fc_1, 50)
    net = tf.layers.dense(fc_2, num_classes,activation='softmax')
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
    return net
